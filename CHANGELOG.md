...
- feat: Argument spec implementation for tlog role (#242)

### Bug Fixes

- fix: Select ini_file module by managed node python version (#240)
- fix: Move ini_file calls to a shared include task file (#250)

### Other Changes

- ci: use commit hash for github action, add persist-credentials false [citest_skip] (#237)
- ci: refactor build_docs so that pandoc runs in isolated read-only job [citest_skip] (#238)
- ci: use exact match for systemroller instead of contains [citest_skip] (#239)
- ci(build_docs): fix pandoc container syntax [citest_skip] (#241)
...
- ci: [citest_skip] Bump codespell-project/actions-codespell from 2.1 to 2.2 (#244)
- ci: [citest_skip] Bump actions/download-artifact from 7.0.0 to 8.0.1 (#245)
- refactor: Relax collection constraints, gate vendored modules by python version, update ci versions [citest_skip] (#246)
- ci: do not run ci tests by default, require citest comment or label [citest_skip] (#247)
- ci: replace weekly_ci with periodic_ci, stagger schedules [citest_skip] (#248)
- ci: update status when action triggered by issue comment (#249)
...
- feat: Write roles fingerprints to /var/log/sysroles.jsonl [citest_skip] (#233)

### Other Changes

- ci: Bump actions/checkout from 6 to 7 (#226)
- ci: Use our own pr_title_lint.py instead of NPM commitlint [citest_skip] (#227)
- ci: bump tox-lsr version to 3.20.0 to fix tox 4.58 api breakage [citest_skip] (#228)
- ci: Add support for Fedora 44 and drop Fedora 42 - use ansible-core 2.21 [citest_skip] (#230)
- ci: Bump actions/setup-python from 6 to 7 (#231)
- ci: ensure dependabot updates do not invoke ci tests [citest_skip] (#232)
...
- refactor: use ansible.posix 2.1.X for EL7 compatibility [citest_skip] (#221)
- ci: Add config file for CodeRabbit with custom rules (#222)
- ci: Skip reviews for PRs with [citest_skip] in the title (#223)
- test: remove unused files (#224)
...
- ci: Bump actions/github-script from 8 to 9 (#218)
...
- ci: Bump ansible/ansible-lint from 25 to 26 (#199)
- ci: skip most CI checks if title contains citest skip [citest_skip] (#200)
- ci: implement citest skip for push events [citest_skip] (#201)
- ci: skip most CI checks if title contains citest skip [citest_skip] (#202)
- ci: ansible-lint - remove .collection directory from converted collection [citest_skip] (#203)
- ci: tox-lsr version 3.15.0 [citest_skip] (#204)
- ci: Add Fedora 43, remove Fedora 41 from Testing Farm CI (#205)
- ci: Bump actions/upload-artifact from 6 to 7 (#207)
- ci: tox-lsr 3.17.0 - container test improvements, use ansible 2.20 for fedora 43 [citest_skip] (#209)
- ci: tox-lsr 3.17.1 - previous update broke container tests, this fixes them [citest_skip] (#210)
- ci: fix yum repos to use devel site instead of old site name [citest_skip] (#212)
- ci: update header for run_role_with_clear_facts [citest_skip] (#213)
- ci: Comply with Ansible partner certification checking [citest_skip] (#214)
- ci: ansible-lint requires dependencies to be installed [citest_skip] (#215)
